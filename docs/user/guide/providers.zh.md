...
响应中包含 `token`、`expires_at` 时间戳，以及标明你的账户所用主机的 `endpoints.api` 值——个人订阅为 `https://api.individual.githubcopilot.com`，企业订阅则是对应的企业地址。把 `token` 存入 `$DSH_HOME/.credentials.yaml` 的 `refs:` 一节：
...
然后在 `$DSH_HOME/settings.yaml` 中声明该路由：
